    ...
    for mfg_name, settings in config.items():
        ...
        def process_cell_strict(row, col_name, mfg):
            ...
            if col_name == "Glasses_other_info":
                if mfg == "safilo":
                    if (
                        pd.notna(row.get("Glasses_model"))
                        and "FLEX" in str(row["Glasses_model"]).upper()
                    ):
                    ...
                    if raw_info == "X":
                        final_values.add("Flex")
                    if (
                        pd.notna(row.get("Glasses_collection"))
                        and str(row["Glasses_collection"]).strip().upper() == "X"
                    ):
                    ...
                    if pd.notna(row.get("Family_descriptions_raw")):
                        if (
                            "double bridge"
                            in str(row["Family_descriptions_raw"]).lower()
                        ):
            ...
            # 🔥 NEW: GLASSES LENS EFFECT ENGINE 🔥
                ...
                if mfg == "safilo":
                    ...
                    if raw_eff and raw_eff.lower() != "nan":
                        ...
                        if not matched:
                            st.session_state.unmapped_values.add(
                                f"Luxottica -> {col_name} (Keyword Search): '{raw_eff}'"
                            )
            ...
            # --- 👓 GLASSES SHAPE ENGINE ---
                ...
                if raw_shape and raw_shape.lower() != "nan":
                    ...
                    if col_name in VALUE_TRANSLATOR:
                        ...
                        lower_dict = {
                            str(k).lower(): v for k, v in translation_dict.items() if k
                        }
        ...
        for target_col in new_df.columns:
            if target_col in VALUE_TRANSLATOR or target_col in [
                ...
                new_df[target_col] = new_df.apply(
                    lambda row: process_cell_strict(row, target_col, mfg_name), axis=1
                )
        ...
        if not new_df.empty:
            ...
            temp_col = new_df.apply(
                lambda row: assemble_name_and_parts(row, mfg_name), axis=1
            )

            # 2. Explicitly extract each piece into its own column
            # This bypasses the Pandas 'Columns must be same length as key' check
            new_df["Assembled_Name"] = temp_col.apply(
                lambda x: x[0] if isinstance(x, (list, tuple)) else ""
            )
            new_df["Extracted_Model"] = temp_col.apply(
                lambda x: x[1] if isinstance(x, (list, tuple)) else ""
            )
            new_df["Extracted_Color"] = temp_col.apply(
                lambda x: x[2] if isinstance(x, (list, tuple)) else ""
            )
        ...
        for dim_col in dimension_cols:
            if dim_col in new_df.columns:
                ...
                def round_dimension(val):
                    ...
                    return str(
                        val
                    ).strip()  # Fallback to original if math completely fails
