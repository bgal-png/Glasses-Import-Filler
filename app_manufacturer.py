...
# ⚠️ Remember to change your password in Supabase later!
...
# ==========================================
@st.cache_data(show_spinner=False, ttl=3600) # Caches the data for 1 hour so it's lightning fast
...
if master_db.empty:
    st.warning("⚠️ Database is empty or unreachable. Please check your connection.")
    st.stop()
...
if search_btn and search_ean:
    ...
    if clean_search in master_db.index:
        ...
        st.error(f"❌ EAN '{search_ean}' was NOT found in the cloud database.")
...
if uploaded_file:
    ...
    if target_barcode_col not in target_df.columns:
        st.error(f"❌ Could not find '{target_barcode_col}' in your file. Found: {list(target_df.columns)}")
        st.stop()
    ...
    if st.button("🚀 Run Auto-Filler", type="primary"):
        with st.spinner("Matching barcodes and pouring data..."):
            ...
            if found_sport_glasses:
                st.warning("⚠️ **Heads Up:** We found 'Sport glasses' in this batch and labeled them as 'Ski goggles' in the Meta Description. Please double-check!")
